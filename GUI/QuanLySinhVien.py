...
class UI_QuanLySinhVien(object):
        ...
        def setupUi(self, MainWindow):
                ...
                icon = QtGui.QIcon()
                icon.addPixmap(QtGui.QPixmap("ui\\../image/moon_symbol_50px.png"),
                ...
                icon1 = QtGui.QIcon()
                icon1.addPixmap(QtGui.QPixmap("ui\\image/time_50px.png"),
                                QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
                ...
                icon2 = QtGui.QIcon()
                icon2.addPixmap(QtGui.QPixmap("ui\\../image/close_50px.png"),
                                QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
                ...
                icon3 = QtGui.QIcon()
                icon3.addPixmap(QtGui.QPixmap("ui\\../image/subtract_50px.png"),
                                QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
                ...
                "}")
                self.label_6.setAlignment(QtCore.Qt.AlignmentFlag.AlignBottom |
                                                QtCore.Qt.AlignmentFlag.AlignLeading | QtCore.Qt.AlignmentFlag.AlignLeft)
                self.label_6.setObjectName("label_6")
                self.cmbKhoa = QtWidgets.QLineEdit(parent=self.frame)
                ...
                self.cmbLop = QtWidgets.QLineEdit(parent=self.frame_2)
                ...
                self.cmbGioiTinh = QtWidgets.QLineEdit(parent=self.frame_2)
                ...
                self.dateNgaySinh = QtWidgets.QLineEdit(parent=self.frame_2)
        ...
        def getDataRow(self):
                ...
                self.cmbLop.setText(self.tbwSV.item(cr, 2).text())
                self.txtCMND.setText(self.tbwSV.item(cr, 3).text())
                self.cmbGioiTinh.setText(self.tbwSV.item(cr, 4).text())

                self.dateNgaySinh.setText(self.tbwSV.item(cr, 5).text())
                self.txtEmail.setText(self.tbwSV.item(cr, 6).text())
                self.txtSDT.setText(self.tbwSV.item(cr, 7).text())
                self.cmbKhoaHoc.setText(self.tbwSV.item(cr, 8).text())

                malop = self.tbwSV.item(cr, 2).text()
                connDb = ConnectDatabase()
                conn = connDb.Connect()
                cursor = conn.cursor()
                query = "SELECT tenkhoa FROM `khoa` INNER JOIN lop ON khoa.makhoa = lop.makhoa WHERE lop.malop = '"+ malop +"'"
                cursor.execute(query)
                row = cursor.fetchone()
                self.cmbKhoa.setText(row[0])
        ...
        def add(self):
                ...
                malop = self.cmbLop.text()
                cmnd = self.txtCMND.text()
                gioitinh = self.cmbGioiTinh.text()
        ...
        def update(self):
                ...
                sodienthoai = self.txtSDT.text()
                khoahoc = self.cmbKhoaHoc.text()
                #print(masinhvien, hoten, malop, cmnd, gioitinh, date, email, sodienthoai, khoahoc)
                sv = SinhVien(masinhvien, hoten, malop , cmnd, gioitinh, date,email ,sodienthoai,khoahoc)
                if(svBUS.update(sv)):
        ...
        def show_camera(self):
                ...
                self.camera = Camera(id=self.ma)
                self.camera.show()

        def show_image(self):
                # Tạo một đối tượng MainWindow để hiển thị ảnh từ folder
                #print('Đang lấy ảnh sv'+ self.ma)
                self.image = ViewImage(id=self.ma)
                self.image.show()
